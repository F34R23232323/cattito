...
funny = [
    ...
    "absolutely not",
    "cattito not responding, try again later",
    "you cant",
    "can you please stop",
    "try again",
    "403 not allowed",
...
# cattito auto-claims in the channel user last ran /vote in
...
# ============================================================================
# CORRECTED HELPER FUNCTIONS - Using Proper catpg Syntax
# Replace these in your main.py
# ============================================================================

async def check_blacklist_slash(interaction: discord.Interaction) -> bool:
    """
    Check if a user/guild/channel is blacklisted.
    Returns True if blacklisted, False if ok.
    """
    try:
...
async def on_message_dev_commands(message: discord.Message):
    ...
    # View blacklist
        ...
        for entry in entries[:25]:
            try:
                blacklisted_by = await bot.fetch_user(entry.blacklisted_by)
...
async def on_message(message: discord.Message):
    ...
    # NEW: Check blacklist immediately
    if await check_message_blacklist(message):
        return

    # NEW: Handle dev commands
    ...
    if text.lower() == "cat":
        ...
        if not channel or not channel.cat or channel.cat in temp_catches_storage or user.timeout > time.time():
            ...
            try:
                ...
                if user.perks:
                    if user.catnip_active < time.time():  # Assuming this is a timestamp comparison
                        if not user.catnip_active:        # or user.catnip_active != True
                            user.catnip_active = False    # <-- use False instead of 1
                ...
                if random.randint(0, 10) == 0 and user.total_catches > 50 and not user.dark_market_active:
                    ...
                    button = Button(label="John Discord 🤠", url="https://discord.gg/BbDXVm4YTg")
...
async def on_guild_join(guild):
    ...
    if not bot.user or bot.user.id == 1387860417706987590:
        unofficial_note = ""
    try:
...
async def help(message):
    ...
    embed2 = (
        ...
        .add_field(
            ...
            value='cattito has various other mechanics to make fun funnier. You can collect various `/achievements`, for example saying "i read help", progress in the `/battlepass`, or have beef with the mafia over catnip addiction. The amount you worship is the limit!',
            inline=False,
        )
        .add_field(
            name="Other features",
        ...
        .set_footer(
            text=f"cattito by Milenakos, {discord.utils.utcnow().year}",
            icon_url="https://wsrv.nl/?url=raw.githubusercontent.com/milenakos/cat-bot/main/images/cat.png",
        )
    )
...
async def credits(message: discord.Interaction):
    ...
    embedVar = discord.Embed(title="cattito", color=Colors.brown, description=gen_credits).set_thumbnail(
...
async def info(message: discord.Interaction):
    embed = discord.Embed(title="cattito Info", color=Colors.brown)
    try:
...
@bot.tree.command(description="Confused? Check out the cattito Wiki!")
...
@bot.tree.command(description="Read The cattito Times™️")
...
@bot.tree.command(description="(ADMIN) Change cattito avatar")
@discord.app_commands.default_permissions(manage_guild=True)
...
you must include the placeholders exactly like they are shown below, the values will be replaced by cattito when it uses them.
that being:
...
@bot.tree.command(description="(ADMIN) enable/disable cattito's reactions")
@discord.app_commands.default_permissions(manage_guild=True)
...
async def catalogue(message: discord.Interaction):
    ...
    # Split into pages of up to 25 fields (Discord limit)
    pages = [fields[i:i + 25] for i in range(0, len(fields), 25)]
    embeds = []
    for idx, page in enumerate(pages):
        embed = discord.Embed(
            title=f"{get_emoji('staring_cat')} The Catalogue" + (f" (Page {idx + 1}/{len(pages)})" if len(pages) > 1 else ""),
            color=Colors.brown,
        )
...
You can earn rain minutes by:
- Reaching Battlepass level 30 each season (+2 minutes)
- Leveling up in Catnip (+1-5 minutes per level)
- Winning special quests and events

As a bonus, supporters get access to /editprofile command!
...
@bot.tree.command(description="Check someone's rain minutes")
...
async def raincheck(
    ...
    user_db = await User.get_or_create(user_id=user.id)
    ...
    if user_db.rain_minutes is None:
        user_db.rain_minutes = 0
        await user_db.save()

    bonus = ""
    if profile.rain_minutes > 0:
        bonus = f" (+**{profile.rain_minutes}** bonus minutes)"


    embed = discord.Embed(
        title="☔ Cat Rains",
        description=f"{user.mention} has **{user_db.rain_minutes}** minutes{bonus}",
        color=Colors.brown
    )
...
if config.DONOR_CHANNEL_ID:
    ...
    async def customcat(message: discord.Interaction, name: Optional[str], image: Optional[discord.Attachment], amount: Optional[int]):
        ...
        if not user.premium and user.rain_minutes < 10:
            await message.response.send_message(
                "👑 This feature is supporter-only!\nBuy anything from cattito Store to unlock custom cats! (or earn 10+ rain minutes)",
                ephemeral=True,
            )
            return
...
# FIXED BLESS COMMAND - Add this to main.py
    ...
    async def bless(message: discord.Interaction):
        ...
        async def regen(interaction):
            ...
            view = View(timeout=VIEW_TIMEOUT)
            if not is_supporter:
                bbutton = Button(label="Supporter Required! (or earn 10+ rain minutes)", url="https://cattito.fun", emoji="👑")
            else:
                bbutton = Button(
                    emoji="🌟",
...
async def rainshop(message: discord.Interaction):
    ...
    # Discord limits selects to 25 options max
    cat_type_options = []
...
@bot.tree.command(description="vote for cattito")
...
async def cat_fact(message: discord.Interaction):
    facts = [
        "you love cats",
        f"cattito is in {len(bot.guilds):,} servers",
        "cat",
        "cats are the best",
    ]
...
async def set_mafia_offer(level, user):
    if user.catnip_level == 0:
        ...
        user.catnip_rain_reward = 0  # Add this line
        return
...
async def catnip(message: discord.Interaction):
    ...
    async def gen_main():
        ...
        # Display pending rain reward
        if user.catnip_level > 0 and user.catnip_rain_reward > 0 and user.hibernation:
            desc += f"\n☔ **Rain Reward**: {user.catnip_rain_reward} minutes upon level completion"
        cat_type = user.catnip_price
        amount = user.catnip_amount
...
async def leaderboards(
    ...
    async def lb_handler(interaction, type, do_edit=None, specific_cat="All"):
        ...
        if type == "Cats":
            ...
            for i in cats[:24]:  # 24 + "All" = 25 max
...
@bot.tree.command(description="(ADMIN) Give packs to people")
@discord.app_commands.default_permissions(manage_guild=True)
...
@bot.tree.command(description="(HIGH ADMIN) [VERY DANGEROUS] Reset all cattito data of this server")
...
# cattito uses glitchtip (sentry alternative) for errors, here u can instead implement some other logic like dming the owner
